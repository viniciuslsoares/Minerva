...
class GradientDataset(SupervisedReconstructionDataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f'tamanho da imagem data[0]: {image_aux.shape}')
        # gradient = generate_gradient(image_aux.shape)[:,:,1]        # Gera o gradiente no mesmo shape e escolhe o vertical
        # print(f'tamanho do gradiente: {gradient.shape}')
        # image = np.concatenate([np.expand_dims(image_aux, axis=0), np.expand_dims(gradient, axis=0)], axis=0)   # Concatena o dado com o gradiente
        # mask = data[1].squeeze()
        # print(f'tamanho da máscara: {mask.shape}')
